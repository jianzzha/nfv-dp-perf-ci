...
openstack overcloud roles generate -o $PWD/roles_data.yaml Controller ComputeOvsDpdk || { echo "failed to generate roles_data.yaml" && exit 1; }
openstack overcloud container image prepare --namespace=docker-registry.engineering.redhat.com/rhosp13 --env-file=$PWD/docker_registry.yaml --prefix=openstack --tag=${DOCKER_IMG_TAG} -e /usr/share/openstack-tripleo-heat-templates/environments/neutron-ovs-dpdk.yaml -r $PWD/roles_data.yaml
...
nova list | sed -r -n 's/.*compute.*-([0-9]+).*ctlplane=([0-9.]+).*/\2 compute-\1/p' | sudo tee --append /etc/hosts >/dev/null
